  ...
  <div class="container mt-5"> 
    ...
    <div class="card mb-4"> 
      ...
      <div class="card-body d-sm-flex justify-content-between">
        <h4 class="mb-2 mb-sm-0 pt-1"> <a href="{{url('admin/dashboard')}}">{{ __('Admin')}}</a> <span>/</span> <a href="{{url('admin/categories')}}">{{$page_title}}</a> <span>/</span> <span>{{ __('Create')}}</span> </h4>
      </div>
    </div>
    <!-- Heading --> 
        ...
        <div class="card mb-4"> 
          ...
          <div class="card-body">
            <form class="eco_form" method="post">
              ...
              <div class="form-group col-md-6">
                <label>{{ __('Name')}}</label>
                <input type="text" class="form-control" name="name" placeholder="{{ __('Category Name')}}" value="{{old('name')}}">
              </div>
			  
			  <div class="form-group col-md-6">
                <label>{{ __('Category Description')}} : <small class="text-muted">[{{ __('Optional')}}]</small></label>
				<textarea name="catgdesc" class="form-control" placeholder="{{ __('Write seo meta desciption of the category here')}}">{{old('catgdesc')}}</textarea>
			  ...
			  <div class="form-group col-md-6">
				...
				<textarea name="catgtags" class="form-control" placeholder="{{ __('tags separated by comma')}}">{{old('catgtags')}}</textarea>
              </div>
			  
              <div class="form-group col-md-6">
                <label>{{ __('Active')}}</label>
                @php
                $selected = old('active',1);
                @endphp
