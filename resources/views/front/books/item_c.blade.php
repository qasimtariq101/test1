...
<div class="ms-item-01">
  <article class="entry-item ct-item-1 style-0{{$rn}}">
  <div class="entry-thumb"> <a href="{{$b->url}}"> <img class="lazyload" src="{{ url('images/default.png') }}" data-src="{{$b->thumbnail_f}}" alt="{{$b->title_f}}"> </a>
    <div class="entry-content">
      ...
      <div class="kopa-rating">
        <div class="star-rating" title="{{ __('Average Rating')}}"> 
          <span style="width: {{($b->average_rating * 100) / 5}}%;"> </span> 
        </div>
        </div>
        @endif
      </header>
    ...
    <footer>
      ...
      <div class="ft-wrap style-02">
        <ul>
          <li> <a class="add_to_favorite" data-pid="{{$b->id}}" data-action="add" title="{{ __('Add to favorite')}}"> <i class="fa  @if(in_array($b->id,$favorites))  fa-heart @else   fa-heart-o @endif"></i> <span>{{ __('Add to favorite')}}</span> </a> </li>
          <li> <a href="{{$b->url}}"> <i class="ti-new-window"></i> </a> </li>
        </ul>
      </div>
    </footer>
    </article>
