...
class SettingController extends Controller
{
    public function edit()
    {
        ...
        return view('admin.settings.index', compact('settings'))->with('page_title', __('Settings'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->type == 'general') {
            $validator = Validator::make($request->all(), [
                ...
                'footer_logo'            => 'nullable|mimes:png|max:200',
                ...
                'maintenance_text'      => 'required|max:5000',
                'header_style'          => 'required|numeric|in:1,2',
                'cookie_consent_bar'          => 'required|numeric|in:1,0',
            ...
            Setting::where('key', 'header_style')->update(['value' => $request->header_style]);
            Setting::where('key', 'cookie_consent_bar')->update(['value' => $request->cookie_consent_bar]);
            ...
            if ($request->hasFile('footer_logo')) {
                ...
                $footer_logo_name = str_random(10) . '.png';
                $destinationPath  = 'uploads';
                $footer_logo->move($destinationPath, $footer_logo_name);

                $footer_logo_path = '/' . $destinationPath . '/' . $footer_logo_name;
                Setting::where('key', 'footer_logo')->update(['value' => $footer_logo_path]);
            }
            ...
            $validator = Validator::make($request->all(), [
                'book_auto_approve'               => 'required|numeric|in:0,1',
                'allow_ratings'               => 'required|numeric|in:0,1',
                'public_upload'               => 'required|numeric|in:0,1',
                'public_download'               => 'required|numeric|in:0,1',
                'admin_only_upload'           => 'required|numeric|in:0,1',
                'books_per_page'              => 'required|numeric|min:10|max:500',
                'daily_upload_limit_unauth'   => 'required|numeric|min:0|max:2000',
                'daily_upload_limit_auth'     => 'required|numeric|min:0|max:2000',
                'max_book_upload_size'        => 'required|numeric|max:200',
                'book_page_layout'            => 'required|numeric|in:1,2',
                'upload_time_restrict_auth'   => 'required|numeric',
                'upload_time_restrict_unauth' => 'required|numeric',
                'featured_books_limit'        => 'required|min:3|max:20|numeric',
                'new_books_limit'             => 'required|min:3|max:20|numeric',
                'ebook_storage'             => 'required|in:uploads,ftp,s3',
                'book_overview_allowed_tags'             => 'nullable|max:500',
                'default_books_page_view'      =>' required|in:grid,list',
                'external_link_embed'      =>' required|numeric|in:0,1',
            ]);
            if ($validator->fails()) {
                if($request->ajax()) return alert_messages_admin($validator->errors()->all(),'error');
                return redirect()->back()->with('type', $request->type)->withInput()
            ...
            Setting::where('key', 'default_books_page_view')->update(['value' => $request->default_books_page_view]);
            Setting::where('key', 'external_link_embed')->update(['value' => $request->external_link_embed]);
            Setting::where('key', 'book_overview_allowed_tags')->update(['value' => $request->book_overview_allowed_tags]);
            Setting::where('key', 'book_auto_approve')->update(['value' => $request->book_auto_approve]);
            Setting::where('key', 'max_book_upload_size')->update(['value' => $request->max_book_upload_size]);
            Setting::where('key', 'upload_time_restrict_unauth')->update(['value' => $request->upload_time_restrict_unauth]);
            Setting::where('key', 'upload_time_restrict_auth')->update(['value' => $request->upload_time_restrict_auth]);
            Setting::where('key', 'public_upload')->update(['value' => $request->public_upload]);
            Setting::where('key', 'public_download')->update(['value' => $request->public_download]);
            Setting::where('key', 'admin_only_upload')->update(['value' => $request->admin_only_upload]);
            Setting::where('key', 'books_per_page')->update(['value' => $request->books_per_page]);
            Setting::where('key', 'book_page_layout')->update(['value' => $request->book_page_layout]);
            Setting::where('key', 'daily_upload_limit_auth')->update(['value' => $request->daily_upload_limit_auth]);
            Setting::where('key', 'daily_upload_limit_unauth')->update(['value' => $request->daily_upload_limit_unauth]);
            Setting::where('key', 'featured_books_limit')->update(['value' => $request->featured_books_limit]);
            Setting::where('key', 'new_books_limit')->update(['value' => $request->new_books_limit]);
            Setting::where('key', 'allow_ratings')->update(['value' => $request->allow_ratings]);
            ...
            if ($request->feature_full_screen) {
                $feature_full_screen = 1;
            } else {
                $feature_full_screen = 0;
            }            
            ...
            Setting::where('key', 'publishers_page')->update(['value' => $publishers_page]);
            ...
            Setting::where('key', 'feature_full_screen')->update(['value' => $feature_full_screen]);
        ...
        elseif ($request->type == 'advertisement') {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'social_yt'    => 'nullable|max:255',
				'social_wapp'    => 'nullable|max:255',
            ...
            Setting::where('key', 'social_yt')->update(['value' => $request->social_yt]);
            Setting::where('key', 'social_wapp')->update(['value' => $request->social_wapp]);
        ...
        elseif ($request->type == 'storage') {
                ...
                $validator = Validator::make($request->all(), [
                'ftp_host'    => 'nullable|max:255',
                'ftp_username'    => 'nullable|max:255',
                'ftp_password'    => 'nullable|max:255',
                'ftp_port'   => 'nullable|numeric',
                'ftp_url' => 'nullable|max:1000',
                'aws_s3_key'   => 'nullable|max:255',
                'aws_s3_secret'   => 'nullable|max:255',
                'aws_s3_region'   => 'nullable|max:255',
                'aws_s3_bucket'   => 'nullable|max:255',
                'aws_s3_url'   => 'nullable|max:1000',
            ]);
            if ($validator->fails()) {
                if($request->ajax()) return alert_messages_admin($validator->errors()->all(),'error');
                return redirect()->back()->with('type', $request->type)->withInput()
            ...
            Setting::where('key', 'ftp_host')->update(['value' => $request->ftp_host]);        
            Setting::where('key', 'ftp_username')->update(['value' => $request->ftp_username]);        
            Setting::where('key', 'ftp_password')->update(['value' => $request->ftp_password]);        
            Setting::where('key', 'ftp_port')->update(['value' => $request->ftp_port]);        
            Setting::where('key', 'ftp_url')->update(['value' => $request->ftp_url]);   

            Setting::where('key', 'aws_s3_key')->update(['value' => $request->aws_s3_key]);        
            Setting::where('key', 'aws_s3_secret')->update(['value' => $request->aws_s3_secret]);        
            Setting::where('key', 'aws_s3_region')->update(['value' => $request->aws_s3_region]);        
            Setting::where('key', 'aws_s3_bucket')->update(['value' => $request->aws_s3_bucket]);        
            Setting::where('key', 'aws_s3_url')->update(['value' => $request->aws_s3_url]);        

        }
        else {
            return redirect()->back();
        }
        if($request->ajax()) return alert_messages_admin(__('Settings successfully updated'));
        return redirect()->back()->with('type', $request->type)->withSuccess(__('Settings successfully updated'));
    }
}
