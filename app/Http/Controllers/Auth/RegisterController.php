...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function register(Request $request)
    {
        ...
        if(config('settings.auto_approve_user') == 0) $success = __('Your account successfully created').'. '.__('Please check your email box and follow instructions to verify email address');
        else $success = __('Your account successfully created');

        return $this->registered($request, $user)
                        ?: redirect($this->redirectPath())->withSuccess($success);
    }  
