  ...
  <div class="container mt-5"> 
    ...
    <div class="card mb-4"> 
      ...
      <div class="card-body d-sm-flex justify-content-between">
        <h4 class="mb-2 mb-sm-0 pt-1"> <a href="{{url('admin/dashboard')}}">{{ __('Admin')}}</a> <span>/</span> <a href="{{url('admin/site-languages')}}">{{$page_title}}</a> <span>/</span> <span>{{ __('Edit')}}</span> </h4>
      </div>
    </div>
    <!-- Heading --> 
          ...
          <div class="card-body">
            <form class="eco_form" method="post">
              ...
              <div class="form-group">
                <label>{{ __('Name')}}</label>
                <input type="text" class="form-control" name="name" placeholder="{{ __('Language Name')}}" value="{{old('name',$language->name)}}">
              </div>   
            
              <div class="form-group">
                <label>{{ __('Site Layout') }} </label> <br />
                @php $selected = old('site_layout',$language->site_layout) @endphp
              ...
              <div class="form-group">
                ...
                <a href="{{url('admin/site-languages')}}" class="btn btn-default">{{ __('Cancel')}}</a> 
              </div>
            </form>
          </div>
        </div>
